...
export class Economy {
  ...
  priceFor(item,event,extraMultiplier=1){const q=QUALITY[item.quality]||QUALITY.normal;const traitMult=item.trait?.multiplier||1;return Math.max(1,Math.round(item.basePrice*q.multiplier*traitMult*demandMultiplier(item.itemId,event)*extraMultiplier));}
