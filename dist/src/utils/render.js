...
function drawOlderChest(ctx,s){pxRect(ctx,s.x-3,s.y+s.h-3,s.w+8,5,'#2a1c1466');pxRect(ctx,s.x,s.y+8,s.w,s.h-8,'#4b2f20');pxRect(ctx,s.x+4,s.y+5,s.w-10,s.h-16,'#725037');pxRect(ctx,s.x+4,s.y,s.w-10,10,'#8a6241');pxRect(ctx,s.x+s.w-7,s.y+8,7,s.h-10,'#3c281c');pxRect(ctx,s.x+s.w/2-4,s.y+11,8,6,'#c08c45');pxRect(ctx,s.x-16,s.y+s.h-10,11,7,'#35291f');pxRect(ctx,s.x+7,s.y-7,22,3,'#77736b');pxRect(ctx,s.x+27,s.y-10,5,11,'#67452b');}
function drawYoungerChest(ctx,s){pxRect(ctx,s.x-3,s.y+s.h-3,s.w+8,5,'#2a1c1466');pxRect(ctx,s.x,s.y+8,s.w,s.h-8,'#493528');pxRect(ctx,s.x+4,s.y+5,s.w-10,s.h-16,'#6a4b35');pxRect(ctx,s.x+4,s.y,s.w-10,10,'#826149');pxRect(ctx,s.x+s.w-7,s.y+8,7,s.h-10,'#3c2a20');pxRect(ctx,s.x+s.w/2-4,s.y+11,8,6,'#c6a05c');pxRect(ctx,s.x+5,s.y-8,24,5,'#967a62');pxRect(ctx,s.x+32,s.y-6,15,4,'#5b4938');}
