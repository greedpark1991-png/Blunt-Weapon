import { CUSTOMER_ARCHETYPES, ITEMS, RUDE_TYPES, RUDE_TYPE_ORDER } from '../data/GameData.js';
...
const typePoolByTime=minute=>{const m=minute%1440;if(m<540)return['villager','guard'];if(m<720)return['villager','guard','hunter'];if(m<1020)return['adventurer','villager','mercenary','hunter'];if(m<1260)return['adventurer','mercenary','guard','hunter'];return['villager','guard'];};
...
export class CustomerSystem{
  constructor(day,state,saved){this.day=day;this.visitors=saved?.visitors?[...saved.visitors]:[];this.nextSpawn=saved?.nextSpawn??22;this.nextId=saved?.nextId??1;this.state=state||{};this.scripted={knight:false,returningKnight:false,rudeGuarantee:false,closingRude:false,...(saved?.scripted||{})};this.wasOpen=saved?.wasOpen??false;this.rudeCount=saved?.rudeCount??0;}
  ...
  shouldSpawnRude(minute){if(this.day<2||this.rudeCount>=2)return false;const base=this.rudeCount===0?.11:.035;return Math.random()<base;}
  ...
  spawnRude(minute,guaranteed=false,forcedType=null){const rudeType=this.pickRudeType(forcedType),def=RUDE_TYPES[rudeType]||RUDE_TYPES.compare,purchases=this.state.customerHistory||[];let itemId=['sword','shield','dagger','longsword'][Math.floor(Math.random()*4)],name=def.name,purchaseRef=null;
    if(['durability','refund'].includes(rudeType)&&purchases.length){purchaseRef=purchases[Math.floor(Math.random()*Math.min(8,purchases.length))];itemId=purchaseRef.itemId;name=purchaseRef.customerName||def.name;}
    if(rudeType==='credit'){name='허풍쟁이 베른';itemId='sword';}
    if(rudeType==='closing')itemId='longsword';if(rudeType==='changes')itemId='dagger';
    this.rudeCount++;return this.spawn({type:'rude',name,mode:'direct',itemId,preferences:[itemId],budget:.9,patience:175,rude:true,rudeType,purchaseRef,guaranteed,line:def.line},minute);
  }
  spawnRandom(minute,event){if(this.shouldSpawnRude(minute))return this.spawnRude(minute);const pool=typePoolByTime(minute);let type=event?.visitorType&&Math.random()<.34?event.visitorType:pool[Math.floor(Math.random()*pool.length)];const a=CUSTOMER_ARCHETYPES[type]||CUSTOMER_ARCHETYPES.villager;const r=Math.random();let mode=r<.68?'general':r<.86?'direct':'commission';const prefs=[...a.preferences],itemId=prefs[Math.floor(Math.random()*prefs.length)],purpose=mode==='direct'&&type==='adventurer'&&Math.random()<.55;return this.spawn({type,name:a.name,mode,itemId,preferences:prefs,budget:a.budget,patience:a.patience,purpose,line:a.line},minute);}
  spawnScripted(kind,minute){if(kind==='knight')return this.spawn({type:'knight',name:'기사 로데릭',mode:'direct',itemId:'sword',preferences:['sword','longsword'],budget:1.6,patience:190,knight:true,line:'이번 원정에 들고 갈 철검 한 자루가 필요하오. 오래 버틸 물건이면 좋겠군.'},minute);const prev=this.state.knightPurchase;return this.spawn({type:'knight',name:'기사 로데릭',mode:'direct',itemId:'longsword',preferences:['longsword','shield','sword'],budget:1.7,patience:200,knight:true,returning:true,line:`전에 사 간 ${prev?.itemName||'무기'}이 꽤 쓸 만했소. 이번에는 장검을 하나 보고 싶군.`},minute);}
  spawn(base,minute){const idx=this.activeCount()%4,p=base.mode==='general'?BROWSE_POS[idx]:COUNTER_POS[idx],v={id:`C${this.nextId++}`,...base,x:598,y:326,targetX:p.x,targetY:p.y,status:'walking',acceptedItem:null,arrivedMinute:minute,bubble:'',bubbleT:0};this.visitors.push(v);return v;}
  ...
  toJSON(){return{visitors:this.visitors.map(v=>({...v})),nextSpawn:this.nextSpawn,nextId:this.nextId,scripted:{...this.scripted},wasOpen:this.wasOpen,rudeCount:this.rudeCount};}
}
