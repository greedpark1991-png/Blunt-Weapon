export class DisplaySystem {
  constructor(data){
    this.capacity=Math.max(1,data?.capacity||6);
    this.slots=Array.from({length:this.capacity},(_,i)=>data?.slots?.[i]||null);
  }
  ...
  toJSON(){ return {capacity:this.capacity,slots:this.slots.map(x=>x?{...x}:null)}; }
}
