...
export class Economy {
  ...
  materialPrice(type,event){const base=MATERIAL_PRICES[type];const mult=event?.material?.[type]||1;return Math.round(base*mult);}
  buyMaterial(type,event){const price=this.materialPrice(type,event);if(this.gold<price)return{ok:false,price};this.gold-=price;this.dayStats.materialCost+=price;return{ok:true,price};}
