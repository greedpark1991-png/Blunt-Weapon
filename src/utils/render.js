...
function drawToolWall(ctx){const y=106;for(let i=0;i<5;i++){pxRect(ctx,110+i*23,y,3,18+(i%2)*5,'#4b3423');pxRect(ctx,105+i*23,y+2,13,4,'#77756f');}pxRect(ctx,96,100,132,4,'#3d2a1c');pxRect(ctx,374,98,94,5,'#3d2a1c');for(let i=0;i<6;i++)pxRect(ctx,382+i*13,104,5,8+(i%3)*5,i%2?'#7b6752':'#555a58');}
function drawCounterBackDetails(ctx){pxRect(ctx,468,239,22,11,'#39271b');pxRect(ctx,472,235,14,6,'#a17a43');pxRect(ctx,500,241,22,7,'#d0b57e');pxRect(ctx,529,238,13,10,'#6d3c2a');}
function drawCounter(ctx,s){pxRect(ctx,s.x,s.y+8,s.w,s.h-8,'#3c281b');pxRect(ctx,s.x-4,s.y,s.w+8,12,'#7d5838');pxRect(ctx,s.x+8,s.y+15,s.w-16,13,'#593a24');pxRect(ctx,s.x,s.y+28,30,44,'#3c281b');pxRect(ctx,s.x-4,s.y+27,38,10,'#7d5838');pxRect(ctx,s.x+7,s.y+39,16,27,'#593a24');label(ctx,s);}
...
function drawSign(ctx,s,open){pxRect(ctx,s.x+16,s.y,4,16,'#6d4a2c');pxRect(ctx,s.x+2,s.y+14,34,18,'#3c291b');pxRect(ctx,s.x+4,s.y+16,30,14,open?'#49633e':'#704437');ctx.fillStyle='#f0d7a9';ctx.font='7px "Malgun Gothic", system-ui, sans-serif';ctx.textAlign='center';ctx.fillText(open?'OPEN':'CLOSED',s.x+19,s.y+20);ctx.textAlign='left';label(ctx,s);}
